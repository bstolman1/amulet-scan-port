...
const MemberTraffic = () => {
  ...
  const memberTraffic = templates?.["Splice:DecentralizedSynchronizer:MemberTraffic"] || [];

  const filteredTraffic = memberTraffic.filter((traffic: any) => {
    const searchLower = searchTerm.toLowerCase();
    const contractId = traffic.contractId || "";
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Member Traffic</h1>
          <p className="text-muted-foreground">
            Network member traffic data
          </p>
        </div>
        ...
        <SearchBar
          ...
          placeholder="Search member traffic..."
        />
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {filteredTraffic.map((traffic: any, index: number) => (
