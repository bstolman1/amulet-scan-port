...
CREATE SCHEMA IF NOT EXISTS `${PROJECT_ID}.canton_silver`
OPTIONS (
  location = 'US',
  description = 'Canton Network silver layer - parsed and typed tables'
);

-- Amulet Contracts (from created events)
CREATE OR REPLACE TABLE `${PROJECT_ID}.canton_silver.amulet_contracts`
...
-- Amulet Archives
CREATE OR REPLACE TABLE `${PROJECT_ID}.canton_silver.amulet_archives`
PARTITION BY DATE(effective_at)
...
-- Locked Amulet Contracts
CREATE OR REPLACE TABLE `${PROJECT_ID}.canton_silver.locked_amulet_contracts`
PARTITION BY DATE(effective_at)
...
-- Amulet Lifecycle (create + archive joined)
CREATE OR REPLACE TABLE `${PROJECT_ID}.canton_silver.amulet_lifecycle`
...
CREATE OR REPLACE TABLE `${PROJECT_ID}.canton_silver.daily_supply_metrics`
