#!/usr/bin/env node
/**
 * ACS Snapshot Fetcher - DuckDB B3 Mode
 * 
...
import { setSnapshotTime, bufferContracts, flushAll, getBufferStats, clearBuffers, waitForWrites } from './duckdb-acs-writer.js';
...
async function runMigrationSnapshot(migrationId) {
  ...
  while (true) {
    ...
    const throughput = Math.round(totalContracts / elapsed);
    
    const stats = getBufferStats();
    console.log(`   📦 Page ${page}: +${contracts.length} contracts (total: ${totalContracts.toLocaleString()}, ${throughput}/s, buffer: ${stats.contracts})`);
...
async function runACSSnapshot() {
  console.log('🚀 Starting ACS Snapshot (DuckDB B3 mode → direct Parquet)\n');
  console.log(`⚙️  Configuration:`);
  console.log(`   - Scan URL: ${SCAN_URL}`);
  console.log(`   - Page Size: ${PAGE_SIZE}`);
  console.log(`   - Flush Rows: ${process.env.ACS_FLUSH_ROWS || '50000 (default)'}`);
  console.log(`   - Flush MS: ${process.env.ACS_FLUSH_MS || '30000 (default)'}`);
  console.log('');
  ...
  for (const r of results) {
    ...
    console.log(`   - Time: ${r.totalTime}s (${Math.round(r.totalContracts / parseFloat(r.totalTime))}/s avg)`);
...
// Run
runACSSnapshot().catch(async err => {
