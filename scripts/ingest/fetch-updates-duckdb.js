#!/usr/bin/env node
/**
 * Canton Ledger Ingestion Script - DuckDB B3 Mode
 * 
 * Fetches ledger updates from Canton Scan API and writes directly to Parquet
 * using DuckDB (no intermediate JSONL files).
 */
...
import { bufferUpdates, bufferEvents, flushAll, getBufferStats } from './duckdb-writer.js';
...
async function runIngestion() {
  console.log('🚀 Starting Canton ledger ingestion (DuckDB B3 mode → direct Parquet)\n');
  console.log('   SCAN_URL:', SCAN_URL);
  console.log('   BATCH_SIZE:', BATCH_SIZE);
  console.log('   POLL_INTERVAL:', POLL_INTERVAL);
