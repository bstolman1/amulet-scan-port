...
// GET /api/events/vote-requests/analyze - Analyze duplicate patterns in VoteRequest data
router.get('/vote-requests/analyze', async (req, res) => {
  try {
    const sources = getDataSources();
    if (sources.primarySource !== 'binary') {
      return res.status(400).json({ error: 'Only binary source supported' });
    }

    const templateIndexReady = await isTemplateIndexPopulated();
    if (!templateIndexReady) {
      return res.status(400).json({ error: 'Template index not populated' });
    }

    const voteRequestFiles = await getFilesForTemplate('VoteRequest');
    console.log(`\n🔍 VOTE-REQUEST ANALYZE: scanning ${voteRequestFiles.length} files...`);
    ...
    for (const file of voteRequestFiles) {
      try {
        ...
        for (const e of result.records || []) {
          if (!e.template_id?.includes('VoteRequest')) continue;
          if (e.event_type === 'created') created.push(e);
        }
      } catch { /* skip */ }
    }
    ...
    for (const e of created) {
      const cid = e.contract_id || 'unknown';
      if (!byContractId[cid]) byContractId[cid] = [];
      byContractId[cid].push(e);
    }

    // Group by trackingCid
    const byTrackingCid = {};
    for (const e of created) {
      const tid = e.payload?.trackingCid || 'none';
      if (!byTrackingCid[tid]) byTrackingCid[tid] = [];
      byTrackingCid[tid].push(e);
    }

    // Find what varies within duplicates
    const contractIdCounts = Object.entries(byContractId)
      .map(([id, arr]) => ({ id, count: arr.length }))
      .sort((a, b) => b.count - a.count)
      .slice(0, 20);

    const trackingCidCounts = Object.entries(byTrackingCid)
      .map(([id, arr]) => ({ id: id.substring(0, 40), count: arr.length }))
      .sort((a, b) => b.count - a.count)
      .slice(0, 20);

    // Sample a duplicate group to show what fields vary
    const sampleDuplicateGroup = Object.entries(byContractId)
      .filter(([_, arr]) => arr.length > 1)
      .slice(0, 1)
      .map(([contractId, events]) => ({
        contractId: contractId.substring(0, 50),
        occurrences: events.length,
        effectiveAts: events.map(e => e.effective_at).sort(),
        // Show payload differences
        payloadSamples: events.slice(0, 3).map(e => ({
          effective_at: e.effective_at,
          trackingCid: e.payload?.trackingCid?.substring(0, 30),
          action: e.payload?.action,
          votes: e.payload?.votes?.length || 0,
          requiredNumVotes: e.payload?.requiredNumVotes,
        })),
      }))[0];

    return res.json({
      summary: {
        totalCreatedEvents: created.length,
        uniqueContractIds: Object.keys(byContractId).length,
        uniqueTrackingCids: Object.keys(byTrackingCid).length,
        avgEventsPerContractId: (created.length / Object.keys(byContractId).length).toFixed(2),
      },
      topContractIdDuplicates: contractIdCounts,
      topTrackingCidDuplicates: trackingCidCounts,
      sampleDuplicateGroup,
      conclusion: Object.keys(byContractId).length === created.length
        ? 'Each event has unique contract_id - dedup by contract_id would give 1:1'
        : `contract_id repeats! ${created.length} events → ${Object.keys(byContractId).length} unique contract_ids`,
    });
  } catch (err) {
    console.error('Error analyzing vote requests:', err);
    res.status(500).json({ error: err.message });
  }
});
