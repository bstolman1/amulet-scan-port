  ...
  try {
    ...
    if (templateIndexPopulated) {
      ...
      if (voteRequestFiles.length === 0) {
        ...
        // NOTE: DsoRules scan removed - status is computed from VoteRequest payload:
        // - voteBefore timestamp → determines if expired
        // - votes map → count accepts/rejects to determine executed/rejected
        // This eliminates scanning 70k+ DsoRules files
      }
    } else {
